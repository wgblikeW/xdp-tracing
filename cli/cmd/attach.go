...
func init() {
	...
	attachCmd.Flags().BoolVarP(&aFlags.SKB_MODE, "sk-mode", "S", false, "Install XDP program in SKB (AKA generic) mode")
	attachCmd.Flags().BoolVarP(&aFlags.NATIVE_MODE, "native-mode", "N", false, "Install XDP program in native mode")
	attachCmd.Flags().BoolVarP(&aFlags.HW_MODE, "hw-mode", "H", false, "Install XDP program in hw mode")
	attachCmd.Flags().StringVarP(&aFlags.FileName, "file-name", "f", "../../bpf/xdp_proxy_kern.o", "binary eBPF program path")
	attachCmd.Flags().StringVarP(&globalFlags.DevName, "dev", "d", "eth0", "Operate on device <ifname>")
	attachCmd.MarkFlagRequired("dev")
	attachCmd.MarkFlagRequired("file-name")
}
